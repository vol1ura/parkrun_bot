...
def _build_top_records_count_sync(data: pd.DataFrame, pic: str):
...
async def top_records_count(pic: str):
    ...
    """
    data = pd.DataFrame()
    # Offload CPU-bound plotting to a background thread to avoid blocking the event loop
    return await asyncio.to_thread(_build_top_records_count_sync, data, pic)
