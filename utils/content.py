...
greetings = ['Приветствую, {}!', 'Здравствуй, {}', 'Привет, {}!', 'Привет, бродяга 😉', 'Здоровеньки булы!']
...
help_message = """Все _команды_ доступны через левое нижнее меню.

Боту можно что-нибудь написать. Например, "Бот, Кузьминки" и бот найдёт случайное фото с забега. \
Либо "Бот, диаграмма" и бот построит гистограмму по результатам вашего последнего забега.

Вопросы и предложения по боту и сайту, направляйте @vol1ura (Володин Юра)

🤖 *S95 Bot*, версия 1.5.0"""
...
no_athlete_message = "Через один пробел после команды введите цифры, указанные в вашем ID (без буквы)."

how_to_rotate_labels = """При необходимости надписи можно повернуть, для этого отправьте сообщение \
"бот диаграмма 30", где 30 - угол поворота.
"""
...
input_pin_code = 'Введите проверочный код, который был выслан на эту почту ' \
    '(если нет письма, подождите немного и/или проверьте спам).'
...
password_requirements = 'должен состоять из латинских букв, содержать хотя бы одну заглавную букву, ' \
    'одну строчную, одну цифру и быть не короче 6 символов.'
...
user_without_athlete = 'Вы зарегистрированы, но участник почему-то не привязан или не создан. ' \
    'Напишите @vol1ura, чтобы решить этот вопрос.'
...
Если у вас есть награды _'Лучше дома'_, то они будут удалены. Пересчёт этих наград для нового домашнего забега займёт некоторое время.
"""
...
personal_history_caption = 'Трактовка: равномерность и интенсивность цвета показывают регулярность участия в забегах S95.'
